  ...
  return `
    <div class="page todo-page" style="padding: 24px 20px 100px 20px; max-width: 620px; margin: 0 auto; width: 100%; box-sizing: border-box;">
      ...
      <div style="margin-bottom: 24px;">
        <button id="btn-open-new-todo" class="btn-primary" style="width: 100%; min-height: 48px; border-radius: 14px; font-size: 14px; font-weight: 600;">
...
function renderTodoCard(todo, habits) {
  const isOverdue = todo.dueDate && todo.dueDate < store.getTodayString() && !todo.completed;
  ...
  return `
    <div class="glass-card todo-card" data-id="${todo.id}" style="padding: 16px 18px; margin-bottom: 12px; border-radius: 16px; border: 1px solid var(--border-subtle); display: flex; align-items: center; justify-content: space-between; gap: 14px; opacity: ${todo.completed ? '0.6' : '1'};">
      <div style="display: flex; align-items: center; gap: 14px; flex: 1; min-width: 0;">
        <button class="btn-toggle-todo" data-id="${todo.id}" data-completed="${todo.completed}" style="width: 26px; height: 26px; border-radius: 50%; border: 1.75px solid ${todo.completed ? 'var(--text-primary)' : 'var(--border-subtle)'}; background: ${todo.completed ? 'var(--text-primary)' : 'transparent'}; color: ${todo.completed ? 'var(--bg-primary)' : 'transparent'}; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
          ${iconSVG('check', 13)}
        </button>
        ...
        <div style="min-width: 0; flex: 1;">
          ...
          <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-top: 4px; font-size: 12px; color: var(--text-secondary);">
            ${todo.dueDate ? `
              <span style="color: ${isOverdue ? '#F56565' : 'var(--text-secondary)'}; font-weight: ${isOverdue ? '700' : '400'};">
                📅 ${todo.dueDate} ${todo.time ? `⏰ ${todo.time}` : ''} ${isOverdue ? '(Vencida)' : ''}
              </span>
            ` : ''}

            ${todo.tag ? `
              <span style="background: var(--bg-subtle); border: 1px solid var(--border-subtle); padding: 2px 8px; border-radius: 12px; font-weight: 600; color: var(--text-primary);">
                🏷️ ${todo.tag}
              </span>
            ` : ''}
          </div>
...
function openCreateTodoModal() {
  document.getElementById('create-todo-modal')?.remove();
  const habits = store.getState().habits || [];
  const todayStr = store.getTodayString();

  const modalHtml = `
    <div id="create-todo-modal" style="position: fixed; inset: 0; background: rgba(0,0,0,0.85); backdrop-filter: blur(8px); z-index: 1500; display: flex; align-items: center; justify-content: center; padding: 20px;">
      <div class="glass-card" style="width: 100%; max-width: 480px; padding: 28px; border-radius: 24px; border: 1px solid var(--border-subtle); background: var(--bg-surface); max-height: 88vh; overflow-y: auto;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
          <h3 class="editorial-title" style="font-size: 22px; margin: 0;">Nueva Tarea (To-Do)</h3>
        ...
        <div style="margin-bottom: 16px;">
          ...
          <input type="text" id="todo-name" class="input" placeholder="Ej. Comprar cuaderno, Enviar correo..." style="width: 100%; min-height: 46px;">
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px;">
          <div>
            <label class="form-label">Fecha de Finalización</label>
            <input type="date" id="todo-date" class="input" value="${todayStr}" style="width: 100%; min-height: 44px;">
          </div>
          <div>
            <label class="form-label">Horario (Opcional)</label>
            <input type="time" id="todo-time" class="input" style="width: 100%; min-height: 44px;">
          </div>
        </div>

        <div style="margin-bottom: 16px;">
          <label class="form-label">Etiqueta / Tag (Opcional)</label>
          <input type="text" id="todo-tag" class="input" placeholder="Ej. Trabajo, Estudio, Personal..." style="width: 100%; min-height: 44px;">
        </div>

        <!-- Habit Stacking Dropdown -->
        <div style="margin-bottom: 24px;">
          <label class="form-label">Acumular con un hábito (Opcional)</label>
          <select id="todo-stacked-after" class="input" style="width: 100%; min-height: 44px;">
            <option value="">Ninguno (Tarea independiente)</option>
            ${habits.map(h => `<option value="${h.id}">Acumular después de hábito: "${h.name}"</option>`).join('')}
          </select>
        </div>
  ...
  document.getElementById('btn-save-todo')?.addEventListener('click', async () => {
    ...
    const dueDate = document.getElementById('todo-date')?.value || todayStr;
